...
export default function QuickActions({ item }: { item: TransformedQuickActions }) {
  ...
  return (
    <AOSComponent>
      <section className="quick-actions-bar box py-0" data-aos="fade-up">
          ...
          {item.actions.map((action, index) => {
            ...
            const content = (
              <>
                <div className="mb-2 md:mb-0 md:mr-4 transition-transform duration-300 group-hover:scale-110">
                  <DynamicLucideIcon name={action.icon || 'HelpCircle'} className="w-8 h-8 md:w-10 md:h-10 text-black" />
                </div>
                <span className="family-montserrat text-sm md:text-xl uppercase tracking-widest text-black">
                  {action.label}
                </span>
              </>
            )

            const className = "flex-1 flex flex-col md:flex-row items-center justify-center py-6 md:py-10 transition-all duration-300 hover:bg-gray-50 group text-black"
            ...
            return (
              <React.Fragment key={index}>
                ...
                {!isLast && (
                  <div className="h-10 md:h-16 w-[1px] bg-black opacity-20 self-center"></div>
                )}
              </React.Fragment>
            )
          })}
